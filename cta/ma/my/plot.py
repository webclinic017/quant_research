...
def plot_stock(fig, row, col, pos, df_data, df_data_market_value, df_buy_trades, df_sell_trades):
    ...
    ax_fund_accumulate.plot(df_data.index, df_data.ma, color='#6495ED', linestyle='--', linewidth=1)

    #
    ax_max_drawdown = ax.twinx()
    ax_max_drawdown.plot(df_data.index, df_data.max_drawdown, color='red', linestyle='--', linewidth=1)
