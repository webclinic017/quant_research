...
class PyramidStrategy(Strategy):
    ...
    def next(self, today, next_trade_date):
        ...
        diff = s_daily_fund.diff_percent - self.current_grid_position
        grid_num = abs(s_daily_fund.diff_percent // self.GRID_HEIGHT)
        ...
        if s_daily_fund.diff_percent < 0 and diff < 0 and grid_num>=1:
            ...
            logger.debug("[%s] %s 距离均线%.1f%% %d个格,超过了历史%.1f%%,买入%.1f份",
                         date2str(today),
                         s_daily_fund.code,
                         s_daily_fund.diff_percent * 100,
                         grid_num,
                         self.current_grid_position*100,
                         positions)

            # 扣除手续费后，下取整算购买份数
            # 追加投资
            if self.broker.buy(s_daily_fund.code, next_trade_date, position=positions):
                self.current_grid_position = - grid_num * self.GRID_HEIGHT

        # 在均线之上，且，超过之前的高度，就卖
        if s_daily_fund.diff_percent > 0 and diff > 0 and grid_num>=1:
            positions = self.policy.calculate(grid_num)
            logger.debug("[%s] %s 距离均线%.1f%% %d个格,超过了历史%.1f%%,卖出%.1f份",
                         date2str(today),
                         s_daily_fund.code,
                         s_daily_fund.diff_percent * 100,
                         grid_num,
